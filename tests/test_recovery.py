...
def sample_journals() -> list[TransactionJournal]:
    j1 = TransactionJournal(
        tx_id="tx_old",
        timestamp=1000.0,
        status="planned",
        entries=[]
    )
    j2 = TransactionJournal(
        tx_id="tx_new",
        timestamp=2000.0,
        status="executing",
        entries=[]
    )
    j3 = TransactionJournal(
        tx_id="tx_committed",
        timestamp=3000.0,
        status="committed",
        entries=[]
...
def test_rollback_journal(sample_journals: list[TransactionJournal]) -> None:
    ...
    with patch("rv.services.recovery.TransactionContext") as mock_ctx_cls, \
...
def test_cli_recover_auto_rollback(sample_journals: list[TransactionJournal]) -> None:
    ...
    with patch("rv.services.recovery.RecoveryService.list_incomplete_journals", return_value=incomplete), \
